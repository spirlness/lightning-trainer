...
def setup_msvc():
    ...
    从环境变量读取配置:
    - MSVC_PATH: MSVC 安装路径 (默认: C:\\Program Files\\Microsoft Visual Studio\\2022\\Community\\VC)
    - MSVC_VERSION: MSVC 版本号 (默认自动检测)
    - WINDOWS_SDK_PATH: Windows SDK 路径 (默认: C:\\Program Files (x86)\\Windows Kits\\10)
    """
    if sys.platform != "win32":
        return

    # 从环境变量获取路径，支持用户自定义
    vs_path = os.environ.get("MSVC_PATH", r"C:\Program Files\Microsoft Visual Studio\2022\Community")
    sdk_path = os.environ.get("WINDOWS_SDK_PATH", r"C:\Program Files (x86)\Windows Kits\10")
    ...
    if not msvc_version and os.path.exists(msvc_base):
        versions = [d for d in os.listdir(msvc_base) if os.path.isdir(os.path.join(msvc_base, d))]
        if versions:
    ...
    if os.path.exists(sdk_include_base):
        versions = [d for d in os.listdir(sdk_include_base)
...
def main():
    ...
    # 模型参数
    parser.add_argument("--model-name", choices=["convnext_tiny", "resnet18"], default="convnext_tiny")
